...
class Command(BaseCommand):
    command = 'show_pentatonic'
    help = 'show pentatonic neck layout'

    def add_arguments(self, parser):
        parser.add_argument('pentatonic', choices=PENTATONICS)
    ...
    def handle(self, pentatonic: str, string_tunings: List[str], start_fret: int, end_fret):  # type: ignore
        pentatonic_obj = getattr(pentatonics, pentatonic)
        ...
        for index, note in enumerate(strings):
            self.show_string(index, note, end_fret, pentatonic_obj, start_fret)
